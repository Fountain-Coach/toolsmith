...
public final class BwrapRunner: SandboxRunner {
  ...
  public init(
    bwrap: URL = URL(fileURLWithPath: "/usr/bin/bwrap"),
  ...
  public func run(
    ...
    if timedOut {
      throw NSError(
        domain: "BwrapRunner", code: 1, userInfo: [NSLocalizedDescriptionKey: "Process timed out"])
    }
  ...
  func prepareCgroup(limits: CgroupLimits) throws -> URL {
    ...
    if let m = limits.memoryMax {
      try (m + "\n").write(
        to: path.appendingPathComponent("memory.max"), atomically: true, encoding: .utf8)
    }
    if let c = limits.cpuMax {
      try (c + "\n").write(
        to: path.appendingPathComponent("cpu.max"), atomically: true, encoding: .utf8)
    }
    if let p = limits.pidsMax {
      try (p + "\n").write(
        to: path.appendingPathComponent("pids.max"), atomically: true, encoding: .utf8)
    }
    return path
  }
  ...
  func add(pid: Int32, toCgroup path: URL) throws {
    try "\(pid)\n".write(
      to: path.appendingPathComponent("cgroup.procs"), atomically: true, encoding: .utf8)
  }
}
